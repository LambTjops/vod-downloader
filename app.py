...
# Persistent database file location (same directory as app.py)
...
def save_downloaded_items():
    ...
    try:
        ...
        os.makedirs(os.path.dirname(DOWNLOADED_DB_FILE), exist_ok=True)
        with open(DOWNLOADED_DB_FILE, 'w') as f:
            json.dump(DOWNLOADED_ITEMS, f, indent=2)
        print(f"Saved {len(DOWNLOADED_ITEMS)} downloaded items to {DOWNLOADED_DB_FILE}")
    except Exception as e:
...
def mark_item_downloaded(item_id, filename, filepath=None):
    ...
    try:
        ...
        print(f"ERROR: Failed to save download record for {item_id}: {e}")
        return False
...
def streams():
    ...
    if cat_type == "movie":
        ...
        # Add downloaded status to each item (check by item_id)
        for item in data:
            item_id = f"movie:{item.get('stream_id', '')}"
            item['_is_downloaded'] = is_item_downloaded(item_id)
    ...
    if 'episodes' in data:
        for season_num, eps in data['episodes'].items():
            ...
            for ep in eps:
                ...
                ep['_is_downloaded'] = is_item_downloaded(item_id)
                ep['_item_id'] = item_id
...
def mark_downloaded_manual(kind, id):
    ...
    if kind == "movie":
        ...
        # We need to search through series to find this episode
        series_cats = get_xtream_data("get_series_categories")
        found = False
        if isinstance(series_cats, list):
            for cat in series_cats:
                series_list = get_xtream_data("get_series", {"category_id": cat['category_id']})
                if isinstance(series_list, list):
                    for series in series_list:
                        series_info = get_xtream_data("get_series_info", {"series_id": series.get('series_id')})
...
def scan_files_and_match():
    """Scan download directory and try to match files with items"""
    if not os.path.exists(DOWNLOAD_PATH):
        return jsonify({'status': 'error', 'message': 'Download directory does not exist'})
    
    files_found = []
    matched_count = 0
    
    # Get all movies and series to match against
    all_movies = []
    all_series_episodes = []
    
    # Get all movie categories and their movies
    movie_cats = get_xtream_data("get_vod_categories")
    if isinstance(movie_cats, list):
        for cat in movie_cats:
            movies = get_xtream_data("get_vod_streams", {"category_id": cat['category_id']})
            if isinstance(movies, list):
                all_movies.extend(movies)
    
    # Get all series categories and their episodes
    series_cats = get_xtream_data("get_series_categories")
    if isinstance(series_cats, list):
        for cat in series_cats:
            series_list = get_xtream_data("get_series", {"category_id": cat['category_id']})
            if isinstance(series_list, list):
                for series in series_list:
                    ...
                    if 'episodes' in series_info:
                        for season_num, eps in series_info['episodes'].items():
                            for ep in eps:
                                ep['_series_name'] = series_info.get('info', {}).get('name', 'Series')
                                all_series_episodes.append(ep)
    ...
    for filename in os.listdir(DOWNLOAD_PATH):
        ...
        if os.path.isfile(filepath) and os.path.getsize(filepath) > 1024 * 1024:
            ...
            # Try to match with movies
            for movie in all_movies:
                safe_name = sanitize_filename(movie.get('name', ''))
                if safe_name.lower() in base_name.lower() or base_name.lower() in safe_name.lower():
                    item_id = f"movie:{movie.get('stream_id')}"
                    if not is_item_downloaded(item_id):
                        mark_item_downloaded(item_id, filename, filepath)
                        matched_count += 1
                        matched = True
            ...
            # Try to match with series episodes (look for S##E## pattern)
            if not matched:
                season_match = re.search(r'S(\d+)E(\d+)', base_name, re.IGNORECASE)
                if season_match:
                    season = int(season_match.group(1))
                    episode = int(season_match.group(2))
                    
                    for ep in all_series_episodes:
                        if ep.get('season') == season and ep.get('episode_num') == episode:
                            # Check if series name matches
                            series_name = ep.get('_series_name', '')
                            if series_name.lower() in base_name.lower():
                                item_id = f"series:{ep.get('id')}"
                                if not is_item_downloaded(item_id):
                                    mark_item_downloaded(item_id, filename, filepath)
                                    matched_count += 1
    ...
    return jsonify({
        ...
        'files': files_found,
        'message': f'Scanned {len(files_found)} files, matched {matched_count} items'
    })
